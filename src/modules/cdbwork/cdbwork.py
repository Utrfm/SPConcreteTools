...
if sof_cdb_kexist(key1, key2) == 2: # the key exists and contains data
    ie = c_int(0)
    RecLen = c_int(sizeof(func))
    while ie.value == 0:
        ie.value = sof_cdb_get(Index, key1, key2, byref(func), byref(RecLen), 1)
...
def SarsRead(area):
    ...
    print(f'{sof_cdb_kexist(key1, key2)=}')
    if sof_cdb_kexist(key1, key2) == 2: # the key exists and contains data
        ie = c_int(0)
        RecLen = c_int(sizeof(func))
        while ie.value < 2:
